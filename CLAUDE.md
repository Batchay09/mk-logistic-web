...
- **YooKassa webhook** `POST /payments/yookassa/webhook` — `payment.succeeded` (верификация через API, не по телу) → статус PAID → PDF-стикеры на email клиенту + email-уведомление менеджерам. `payment.canceled` → заказ возвращается в корзину (NEW).
- **Неудачная оплата** — заказ не должен виснуть в `AWAITING_PAYMENT` (раньше висел навсегда: из корзины пропал, удалить нельзя, оплатить повторно неоткуда). Три рубежа: webhook `payment.canceled`; сверка `POST /payments/yookassa/sync` (спрашивает у ЮKassa настоящий статус — возврат на `return_url` оплату НЕ доказывает, а webhook может не дойти) при возврате с оплаты и при входе в корзину; кнопка «Оплатить» на карточке заказа. Страница результата — `/payments/result` (опрос до 5 раз с шагом 2 с, пока платёж `pending`). `yookassa_payment_id` при откате НЕ обнуляем — по нему страница результата узнаёт судьбу платежа; откат сверяет id, чтобы запоздавшее «отменён» по прошлой попытке не сбило начатую новую.
