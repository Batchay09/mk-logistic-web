...
- **YooKassa webhook** `POST /payments/yookassa/webhook` — `payment.succeeded` (верификация через API, не по телу) → статус PAID → PDF-стикеры на email клиенту + email-уведомление менеджерам. `payment.canceled` → только логируется, статус заказа не меняется.
- **Неоплаченный заказ = «Ожидает оплаты», как у большинства сервисов** (⚠️ смена модели). Раньше при неудачной оплате заказ висел в `AWAITING_PAYMENT` навсегда, потом я на короткое время возвращал его в корзину (NEW) — Азамат резонно назвал это нелогичным. Итог: непрошедшая оплата **не меняет статус** — заказ остаётся `AWAITING_PAYMENT`, виден в «Активных заказах» (кнопка «Оплатить» в списке и в карточке). Клиент сам отменяет ненужный заказ: `POST /client/orders/{id}/cancel` → CANCELED (перед отменой сверяет платёж с ЮKassa: если уже оплачен — не отменяет, а помечает PAID и отдаёт 409). Возврата в корзину больше нет.
- **Страница результата** `/payments/result` — сверяет платёж через `POST /payments/yookassa/sync` (возврат на `return_url` оплату НЕ доказывает, webhook может отстать). Максимум 2 быстрые проверки (~3 с), чтобы поймать опоздавший `succeeded`; иначе «Оплата не завершена, заказ ждёт оплаты» (не бесконечный спиннер и не «оплата не прошла»). `id` платежа фронт запоминает в `sessionStorage` до редиректа. Сверка статус заказа не откатывает — единственное её изменение состояния — `succeeded` → PAID.
- **`_mark_orders_paid` помечает PAID только заказ в `AWAITING_PAYMENT`** — если клиент успел отменить, а оплата всё же прошла, заказ не воскресает молча: логируется для ручного возврата.
- **Вызовы SDK ЮKassa — только через `_call_yookassa`** (⚠️ грабли): SDK синхронный (requests) и не задаёт сетевой таймаут — прямой вызов из async-обработчика вешает воркер (в проде их 2) до ответа ЮKassa. Хелпер уносит вызов в поток и ограничивает 10 секундами.
...
- **Стикеры: подпись оплаты** — на бумаге «Наличные»/«Безнал» (`_PAYMENT_LABELS`), в QR (10-е поле TSV) — сырое `cash`/`cashless`: это машинный контракт со сканером и Excel-выгрузкой, менять его нельзя. Русская подпись шире прежней латиницы, поэтому кегль строки ужимается под ширину 58 мм.
